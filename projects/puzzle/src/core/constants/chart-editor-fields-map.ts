	...
	children: [
		{
			...
			disabled: false,
			options: [
				CHART_AXES_TYPE_OPTIONS_BY_KEY[ChartAxesTypesEnum.category],
				CHART_AXES_TYPE_OPTIONS_BY_KEY[ChartAxesTypesEnum.time],
		...
		{ label: '启用辅助线', key: 'gridLineEnable', path: 'gridLine.enabled', schemaType: 'select', options: ENABLE_OPTIONS },
		{ label: '辅助线宽度', key: 'gridLineWidth', path: 'gridLine.width', schemaType: 'number', min: 0, max: 100, step: 1 },
		// { label: '辅助线颜色', key: 'gridLineStyleStroke', path: 'gridLine.style.stroke', schemaType: 'color' },
	],
};
...
export const CHART_SCATTER_AXES: EditorChartField = {
	...
	children: [
		...
		{ label: '轴标题', key: 'titleText', path: 'title.text', schemaType: 'text' },
		{ label: '轴标题大小', key: 'titleFontSize', path: 'label.fontSize', schemaType: 'number' },
		{ label: '轴标题颜色', key: 'titleColor', path: 'title.color', schemaType: 'color' },
		{
			label: '标签单位格式化',
			key: 'labelFormatter',
			...
			schemaType: 'select',
			options: [
				{ label: '默认', val: LabelFormatterEnum.default },
				{ label: '长度（cm）', val: LabelFormatterEnum.lengthCm },
				{ label: '质量（kg）', val: LabelFormatterEnum.massKg },
				{ label: '百分比（%）', val: LabelFormatterEnum.percentage },
			],
		},
	],
};
	...
	children: [
		{
			...
			schemaType: 'select',
			disabled: true,
			options: [CHART_AXES_TYPE_OPTIONS_BY_KEY[ChartAxesTypesEnum.ordinalTime], CHART_AXES_TYPE_OPTIONS_BY_KEY[ChartAxesTypesEnum.number]],
		},
		{
			label: '位置',
			key: 'position',
		...
		// 	schemaType: 'text',
		// 	description:
		// 		'控制小数：%.0f - 不显示小数（四舍五入）；\n %.1f - 显示1位小数；%.2f - 显示2位小数。添加单位：前缀：¥%,.2f → ¥1,234.56，后缀：%.1f%% → 12.3%。',
...
// 径向条形图轴设置
export const CHART_RADIAL_COLUMN_AXES: EditorChartField = CHART_NIGHTINGALE_AXES;
...
export const BAR_SERIES: EditorChartField = {
	...
	children: [
		...
		{ label: '颜色', key: 'fill', schemaType: 'color' },
		{ label: '颜色透明度', key: 'fillOpacity', schemaType: 'number', min: 0, max: 1, step: 0.1 },
	],
};
...
export const SCATTER_SERIES: EditorChartField = {
	...
	children: [
		...
		{ label: '标签颜色', path: 'label.color', key: 'labelColor', schemaType: 'color' },
		{
			label: '形状',
			key: 'shape',
			schemaType: 'select',
			options: CHART_MARKER_SHAPE_OPTIONS,
		},
...
export const PIE_SERIES: EditorChartField = {
	...
	children: [
		...
		{
			label: '显示标注标签',
			key: 'calloutLabelEnabled',
			path: 'calloutLabel.enabled',
			schemaType: 'select',
			options: ENABLE_OPTIONS,
		},
		{ label: '标注标签颜色', key: 'calloutLabelColor', path: 'calloutLabel.color', schemaType: 'color' },
		{
			label: '标注标签加粗',
			key: 'calloutLabelFontWeight',
			path: 'calloutLabel.fontWeight',
			schemaType: 'select',
			options: FONT_WEIGHT_OPTIONS,
		},
...
// 地图系列设置
export const MAP_SERIES: EditorChartField = {
	label: '数据系列',
	key: 'series',
	path: 'series',
	schemaType: 'array',
	...
	children: [
		{
			...
			schemaType: 'select',
			options: [
				CHART_SERIES_TYPE_OPTIONS_BY_KEY[ChartTypesEnum.mapShapeBackground],
				CHART_SERIES_TYPE_OPTIONS_BY_KEY[ChartTypesEnum.mapShape],
				CHART_SERIES_TYPE_OPTIONS_BY_KEY[ChartTypesEnum.mapMarker],
			],
		},
		{
			label: '地图级别',
			...
			path: 'levelKey',
			schemaType: 'select',
			belong: [ChartTypesEnum.mapShapeBackground],
			options: [
				{ label: '国', val: 'country' },
				{ label: '省', val: 'province' },
				{ label: '市', val: 'city' },
			],
		},
		{ label: '绘制地图', key: 'topology', disabled: true, schemaType: 'button', belong: [ChartTypesEnum.mapShapeBackground] },
		{ label: '数据源', key: 'data', schemaType: 'button', belong: [ChartTypesEnum.mapMarker] },
		{
			label: '纬度键值',
			key: 'latitudeKey',
			path: 'latitudeKey',
			schemaType: 'text',
			belong: [ChartTypesEnum.mapMarker],
		},
		{
			label: '纬度名称',
			key: 'latitudeName',
		...
		},
		{
			...
			path: 'shape',
			schemaType: 'select',
			description: '因渲染性能，选项不会实时更新，可在预览中查看效果',
			belong: [ChartTypesEnum.mapMarker],
			options: CHART_MARKER_SHAPE_OPTIONS,
		},
		{
